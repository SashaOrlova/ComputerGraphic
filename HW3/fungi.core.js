var Fungi = (function(){
	...
	class Renderable extends Transform {
		...
		draw(){
			for (var index = 0; index < this.vao.length; index++) {
				gl.bindVertexArray(this.vao[index].id);
				if (this.vao[index].isIndexed) {
					gl.drawElements(this.material.drawMode, this.vao[index].count, gl.UNSIGNED_SHORT, 0);
				}
				else {
					gl.drawArrays(this.material.drawMode, 0, this.vao[index].count);
				}
			}
		}
	}
